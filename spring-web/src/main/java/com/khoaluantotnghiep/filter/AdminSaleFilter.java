...
@WebFilter(urlPatterns = { "/quan-tri/hoa-don*", "/quan-tri/hoa-don/*", "/quan-tri/khach-hang*",
		"/quan-tri/khach-hang/*" })
public class AdminSaleFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (user != null && user.getRole().equals("ROLE_ADMIN") && user.getRoleEntity().getCode().equals("ADMIN_ALL")
				|| user != null && user.getRole().equals("ROLE_ADMIN")
						&& user.getRoleEntity().getCode().equals("ADMIN_SALE")) {
